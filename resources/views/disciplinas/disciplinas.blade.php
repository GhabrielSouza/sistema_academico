    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('disciplinas.store') }}" class="p-6">
            ...
            <div class="mt-6">
                ...
                <x-input-error :messages="$errors->userDeletion->get('nome')" class="mt-2" />
            </div>

            <div class="mt-6">
                <x-input-label for="descricao" value="{{ __('Descrição') }}" class="sr-only" />

                <x-text-input id="descricao" name="descricao" type="text" class="mt-1 block w-3/4"
                    placeholder="{{ __('Password') }}" />

                <x-input-error :messages="$errors->userDeletion->get('descricao')" class="mt-2" />
            </div>
